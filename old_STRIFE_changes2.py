...
class STRIFE:
    ...
    def __init__(self, args):
            ...
            if self.Filtered:
                ...
                for idx, row in self.HotspotsDF.iterrows():
                    ...
                    for idx2, row2 in self.FilteredDF.iterrows():
                        if self.preprocessing.vectorDistance(row['position'], row2['position']) < 2:
                            add = 0
                    if add == 1:
        ...
        self.HotspotsDF = self.HotspotsDF.head(4)
        self.HotspotsDF = self.HotspotsDF.reset_index(drop=True)
            ...
            #ie using one of the on the way methods
                if args.iter_type == 'otw':
                    ...
                    for indx in self.topHotspot.index.to_list():
                        ...
                        dists.append(self.preprocessing.vectorDistance(self.topHotspot['position'][indx],self.exitVectorPos))
    ...
    def elaborationsWithoutRefinement(self, counts = True, totalNumElabs = 250, numElabsPerPoint = 250, n_cores = None,index=0, single=True):
        ...
        if single == True:
            ...
            self.elabsTestNoRefineDocksFiltered = self.singleDistances.loc[self.singleDistances['distance'] < 2].drop_duplicates('smiles')
            ...
            self.elabsTestNoRefineDocksFiltered = self.multiDistances.loc[self.multiDistances['distance'] < 2].drop_duplicates('smiles')
            if len(self.elabsTestNoRefineDocksFiltered) == 0:
 ...
 ###########################
        ...
        self.elabsTestNoRefineDocksFiltered =self.multiDistances.loc[self.multiDistances['distance'] < 2].drop_duplicates('smiles').reset_index(drop=True)
